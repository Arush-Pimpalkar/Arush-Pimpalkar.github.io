...
async function loadUpdates() {
  const rowsContainer = document.getElementById("updates-rows");
  if (!rowsContainer) {
    return;
  }

  try {
    const records = await loadUpdatesData();
    updatesEntries = records;
    initUpdatesFilters();
    renderUpdates(rowsContainer, applyUpdatesFilters());
  } catch (error) {
    console.error("Unable to load updates feed.", error);
  }
}

function parseUpdatesCSV(text) {
  const rows = [];
  let currentField = "";
  let currentRow = [];
...
function renderUpdates(container, entries) {
  ...
  groups.forEach(({ year, items }) => {
    ...
    group.className = "updates_year_group";

    const yearCell = document.createElement("div");
    yearCell.className = "updates_year";
    yearCell.textContent = year;
    group.appendChild(yearCell);

    const entriesCol = document.createElement("div");
    entriesCol.className = "updates_year_entries";

    items.forEach((entry, index) => {
      const entryRow = document.createElement("div");
      entryRow.className = "updates_entry";
      entryRow.style.setProperty("--updates-index", index);

      const monthCell = document.createElement("div");
      monthCell.className = "updates_month";
      monthCell.textContent = entry.month;

      const bodyCell = document.createElement("div");
      bodyCell.className = "updates_body";

      const entryHeading = document.createElement("div");
      entryHeading.className = "updates_entry_heading";

      const yearLabel = document.createElement("span");
      yearLabel.className = "updates_year_label";
      yearLabel.textContent = entry.year || "";
      entryHeading.appendChild(yearLabel);

      const monthLabelInline = document.createElement("span");
      monthLabelInline.className = "updates_month";
      ...
      if (entry.headline) {
        ...
        headline.className = "updates_headline";
      ...
      if (entry.image) {
        ...
        if (isVideoAsset(entry.image)) {
          ...
          video.setAttribute(
            "aria-label",
            entry.image_alt || `${entry.month} ${entry.year} updates video`
          );
          ...
          image.alt =
            entry.image_alt || `${entry.month} ${entry.year} updates image`;
          image.loading = "lazy";
...
function initUpdatesFilters() {
  const controls = document.getElementById("updates-controls");
  if (!controls) {
    return;
  }

  // Temporarily hide filters on the updates page.
...
function createFilterButton(label, filterKey, iconClass) {
  ...
  button.type = "button";
  button.className = "updates_filter_btn";
  ...
  button.addEventListener("click", () => {
    ...
    updateUpdatesHeading();
    renderUpdates(document.getElementById("updates-rows"), applyUpdatesFilters());
  });
...
function applyUpdatesFilters() {
  ...
  return updatesEntries.filter((entry) => {
...
function updateUpdatesHeading() {
  const heading = document.getElementById("updates-heading");
  if (!heading) {
    return;
  }
  ...
  if (includeFailures) {
    fragments.push(createHeadingTag("real", "updates_heading-tag"));
  }

  if (includeUnfiltered) {
    fragments.push(createHeadingTag("life", "updates_heading-tag"));
  }

  fragments.push(document.createTextNode("Updates"));
  ...
  heading.classList.add("updates_heading-flash");

  heading.addEventListener(
    "animationend",
    () => heading.classList.remove("updates_heading-flash"),
    { once: true }
  );
}
...
async function loadUpdatesData() {
  ...
  if (loader) {
    const workbookRecords = await loader.readWorkbook("/data/updates.xlsx");
    if (workbookRecords.length) {
      return loader.normalizeRecords(workbookRecords).map(tagEntry);
    }

    const csvText = await loader.readCsv("/data/updates.csv");
    if (csvText) {
      const csvRecords = parseUpdatesCSV(csvText);
  ...
  const csvText = await readCsvFallback("/data/updates.csv");
  if (!csvText) {
    return [];
  }
  const csvRecords = parseUpdatesCSV(csvText);
